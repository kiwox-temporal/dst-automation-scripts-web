...
public class DownloadCommand extends AbstractCommand {
	...
	protected TestResult internalRunTest(String[] args) {
		if (parseCommandLine(args)) {
			...
			ITest test = new LoadTest(false, url, file, iterations);
			return startServer(test);
		}
		TestResult result = new TestResult();
		result.setError(true);
		result.setCode("DST-DOWNLOAD-003");
		return result;
	}
	...
	protected Options buildOptions() {
		...
		options.addOption(Option
				...
				.hasArg()
				.required()
				.desc("URL to access the file (required)")
				.build());
		options.addOption(Option
				.builder(FILE_OPTION)
				.longOpt("filePath")
				.hasArg()
				.required()
				.desc("Path in the device to save the file. The file is deleted after the test (required)")
				.build());
		options.addOption(Option
				...
				.hasArg()
				.required(false)
				.desc("Amount of times to download. Default to " + DEFAULT_ITERATIONS)
				.type(Number.class)
				.build());
		return options;
	}
